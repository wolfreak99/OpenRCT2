...
void NetworkServer::SetEnvironment(IPlatformEnvironment * env)
{
    _env = env;
}
...
bool NetworkServer::BeginClient(const char* host, uint16 port)
{
...
bool NetworkServer::BeginServer(uint16 port, const char* address)
{
    Close();
    if (!Init())
        return false;
...
sint32 NetworkServer::GetMode()
{
    return mode;
}

sint32 NetworkServer::GetStatus()
{
    return status;
}

sint32 NetworkServer::GetAuthStatus()
{
...
uint32 NetworkServer::GetServerTick()
{
    return server_tick;
}

uint8 NetworkServer::GetPlayerID()
{
    return player_id;
}
...
void NetworkServer::UpdateClient()
{
...
std::vector<std::unique_ptr<NetworkPlayer>>::iterator NetworkServer::GetPlayerIteratorByID(uint8 id)
{
...
NetworkPlayer* NetworkServer::GetPlayerByID(uint8 id)
{
...
std::vector<std::unique_ptr<NetworkGroup>>::iterator NetworkServer::GetGroupIteratorByID(uint8 id)
{
...
NetworkGroup* NetworkServer::GetGroupByID(uint8 id)
{
...
const char* NetworkServer::FormatChat(NetworkPlayer* fromplayer, const char* text)
{
...
void NetworkServer::SendPacketToClients(NetworkPacket& packet, bool front, bool gameCmd)
{
...
bool NetworkServer::CheckSRAND(uint32 tick, uint32 srand0)
{
...
void NetworkServer::KickPlayer(sint32 playerId)
{
    for(auto it = client_connection_list.begin(); it != client_connection_list.end(); it++) {
...
void NetworkServer::SetPassword(const char* password)
{
    _password = password == nullptr ? "" : password;
}

void NetworkServer::ShutdownClient()
{
    if (GetMode() == NETWORK_MODE_CLIENT) {
...
std::string NetworkServer::GenerateAdvertiseKey()
{
...
const char *NetworkServer::GetMasterServerUrl()
{
...
NetworkGroup* NetworkServer::AddGroup()
{
...
void NetworkServer::RemoveGroup(uint8 id)
{
...
uint8 NetworkServer::GetGroupIDByHash(const std::string &keyhash)
{
...
uint8 NetworkServer::GetDefaultGroup()
{
    return default_group;
}

void NetworkServer::SetDefaultGroup(uint8 id)
{
    if (GetGroupByID(id)) {
        default_group = id;
    }
...
void NetworkServer::SetupDefaultGroups()
{
...
std::string NetworkServer::BeginLog(const std::string &directory, const std::string &filenameFormat)
{
    utf8 filename[256];
    time_t timer;
    time(&timer);
...
void NetworkServer::AppendLog(const std::string &logPath, const std::string &s)
{
...
void NetworkServer::BeginChatLog()
{
    auto directory = _env->GetDirectoryPath(DIRBASE::USER, DIRID::LOG_CHAT);
    _chatLogPath = BeginLog(directory, _chatLogFilenameFormat);
}

void NetworkServer::AppendChatLog(const std::string &s)
{
...
void NetworkServer::CloseChatLog()
{
}
...
void NetworkServer::Client_Send_TOKEN()
{
...
void NetworkServer::Client_Send_AUTH(const char* name, const char* password, const char* pubkey, const char *sig, size_t sigsize)
{
    std::unique_ptr<NetworkPacket> packet(NetworkPacket::Allocate());
...
void NetworkServer::Client_Send_OBJECTS(const std::vector<std::string> &objects)
{
...
void NetworkServer::Server_Send_TOKEN(NetworkConnection& connection)
{
    std::unique_ptr<NetworkPacket> packet(NetworkPacket::Allocate());
...
void NetworkServer::Server_Send_OBJECTS(NetworkConnection& connection, const std::vector<const ObjectRepositoryItem *> &objects) const
{
...
void NetworkServer::Server_Send_AUTH(NetworkConnection& connection)
{
...
void NetworkServer::Server_Send_MAP(NetworkConnection* connection)
{
...
uint8 * NetworkServer::save_for_network(size_t &out_size, const std::vector<const ObjectRepositoryItem *> &objects) const
{
...
void NetworkServer::Client_Send_CHAT(const char* text)
{
    std::unique_ptr<NetworkPacket> packet(NetworkPacket::Allocate());
...
void NetworkServer::Server_Send_CHAT(const char* text)
{
    std::unique_ptr<NetworkPacket> packet(NetworkPacket::Allocate());
    *packet << (uint32)NETWORK_COMMAND_CHAT;
    packet->WriteString(text);
...
void NetworkServer::Client_Send_GAMECMD(uint32 eax, uint32 ebx, uint32 ecx, uint32 edx, uint32 esi, uint32 edi, uint32 ebp, uint8 callback)
{
    std::unique_ptr<NetworkPacket> packet(NetworkPacket::Allocate());
...
void NetworkServer::Server_Send_GAMECMD(uint32 eax, uint32 ebx, uint32 ecx, uint32 edx, uint32 esi, uint32 edi, uint32 ebp, uint8 playerid, uint8 callback)
{
    std::unique_ptr<NetworkPacket> packet(NetworkPacket::Allocate());
    *packet << (uint32)NETWORK_COMMAND_GAMECMD << (uint32)gCurrentTicks << eax << (ebx | GAME_COMMAND_FLAG_NETWORKED)
...
void NetworkServer::Server_Send_TICK()
{
...
void NetworkServer::Server_Send_PLAYERLIST()
{
    std::unique_ptr<NetworkPacket> packet(NetworkPacket::Allocate());
...
void NetworkServer::Client_Send_PING()
{
    std::unique_ptr<NetworkPacket> packet(NetworkPacket::Allocate());
...
void NetworkServer::Server_Send_PING()
{
...
void NetworkServer::Server_Send_PINGLIST()
{
    std::unique_ptr<NetworkPacket> packet(NetworkPacket::Allocate());
...
void NetworkServer::Server_Send_SETDISCONNECTMSG(NetworkConnection& connection, const char* msg)
{
    std::unique_ptr<NetworkPacket> packet(NetworkPacket::Allocate());
...
void NetworkServer::Server_Send_EVENT_PLAYER_DISCONNECTED(const char *playerName, const char *reason)
{
    std::unique_ptr<NetworkPacket> packet(NetworkPacket::Allocate());
    *packet << (uint32)NETWORK_COMMAND_EVENT;
...
bool NetworkServer::ProcessConnection(NetworkConnection& connection)
{
    sint32 packetStatus;
    do {
...
void NetworkServer::ProcessPacket(NetworkConnection& connection, NetworkPacket& packet)
{
    uint32 command;
    packet >> command;
...
void NetworkServer::ProcessGameCommandQueue()
{
...
void NetworkServer::AddClient(ITcpSocket * socket)
{
...
NetworkPlayer* NetworkServer::AddPlayer(const utf8 *name, const std::string &keyhash)
{
...
std::string NetworkServer::MakePlayerNameUnique(const std::string &name)
{
...
void NetworkServer::Client_Handle_TOKEN(NetworkConnection& connection, NetworkPacket& packet)
{
    utf8 keyPath[MAX_PATH];
    network_get_private_key_path(keyPath, sizeof(keyPath), gConfigNetwork.player_name);
    if (!platform_file_exists(keyPath)) {
...
void NetworkServer::Client_Handle_AUTH(NetworkConnection& connection, NetworkPacket& packet)
{
    uint32 auth_status;
...
void NetworkServer::Server_Client_Joined(const char* name, const std::string &keyhash, NetworkConnection& connection)
{
...
void NetworkServer::Server_Handle_TOKEN(NetworkConnection& connection, NetworkPacket& packet)
{
...
void NetworkServer::Client_Handle_OBJECTS(NetworkConnection& connection, NetworkPacket& packet)
{
...
void NetworkServer::Server_Handle_OBJECTS(NetworkConnection& connection, NetworkPacket& packet)
{
    uint32 size;
    packet >> size;
...
void NetworkServer::Server_Handle_AUTH(NetworkConnection& connection, NetworkPacket& packet)
{
...
void NetworkServer::Client_Handle_MAP(NetworkConnection& connection, NetworkPacket& packet)
{
    uint32 size, offset;
    packet >> size >> offset;
...
bool NetworkServer::LoadMap(IStream * stream)
{
    bool result = false;
    try
    {
...
bool NetworkServer::SaveMap(IStream * stream, const std::vector<const ObjectRepositoryItem *> &objects) const
{
    bool result = false;
...
void NetworkServer::Client_Handle_CHAT(NetworkConnection& connection, NetworkPacket& packet)
{
...
void NetworkServer::Server_Handle_CHAT(NetworkConnection& connection, NetworkPacket& packet)
{
    if (connection.Player) {
...
void NetworkServer::Client_Handle_GAMECMD(NetworkConnection& connection, NetworkPacket& packet)
{
    uint32 tick;
    uint32 args[7];
    uint8 playerid;
...
void NetworkServer::Server_Handle_GAMECMD(NetworkConnection& connection, NetworkPacket& packet)
{
    uint32 tick;
    uint32 args[7];
    uint8 playerid;
...
void NetworkServer::Client_Handle_TICK(NetworkConnection& connection, NetworkPacket& packet)
{
    uint32 srand0;
    uint32 flags;
...
void NetworkServer::Client_Handle_PLAYERLIST(NetworkConnection& connection, NetworkPacket& packet)
{
    uint8 size;
    packet >> size;
...
void NetworkServer::Client_Handle_PING(NetworkConnection& connection, NetworkPacket& packet)
{
    Client_Send_PING();
}

void NetworkServer::Server_Handle_PING(NetworkConnection& connection, NetworkPacket& packet)
{
...
void NetworkServer::Client_Handle_PINGLIST(NetworkConnection& connection, NetworkPacket& packet)
{
    uint8 size;
    packet >> size;
    for (uint32 i = 0; i < size; i++) {
...
void NetworkServer::Client_Handle_SETDISCONNECTMSG(NetworkConnection& connection, NetworkPacket& packet)
{
...
void NetworkServer::Server_Handle_GAMEINFO(NetworkConnection& connection, NetworkPacket& packet)
{
    Server_Send_GAMEINFO(connection);
}

void NetworkServer::Client_Handle_SHOWERROR(NetworkConnection& connection, NetworkPacket& packet)
{
...
void NetworkServer::Client_Handle_GROUPLIST(NetworkConnection& connection, NetworkPacket& packet)
{
    group_list.clear();
    uint8 size;
...
void NetworkServer::Client_Handle_EVENT(NetworkConnection& connection, NetworkPacket& packet)
{
    char text[256];
    uint16 eventType;
    packet >> eventType;
...
void NetworkServer::Client_Send_GAMEINFO()
{
...
void NetworkServer::Client_Handle_GAMEINFO(NetworkConnection& connection, NetworkPacket& packet)
{
...
void network_chat_show_connected_message()
{
    ...
    server.Name = "Server";
    const char * formatted = NetworkServer::FormatChat(&server, buffer);
    chat_history_add(formatted);
}
